...
#ifndef OPEN_SPIEL_GAMES_LEDUC_POKER_10_CARD_H_
#define OPEN_SPIEL_GAMES_LEDUC_POKER_10_CARD_H_
...
class Leduc10CardGame;
class Leduc10CardObserver;

enum ActionType { kFold = 0, kCall = 1, kRaise = 2 };

class Leduc10CardState : public State {
 public:
  explicit Leduc10CardState(std::shared_ptr<const Game> game,
 ...
 private:
  friend class Leduc10CardObserver;
...
class Leduc10CardGame : public Game {
 public:
  explicit Leduc10CardGame(const GameParameters& params);
  ...
  std::shared_ptr<Leduc10CardObserver> default_observer_;
  std::shared_ptr<Leduc10CardObserver> info_state_observer_;
...
#endif  // OPEN_SPIEL_GAMES_LEDUC_POKER_10_CARD_H_
