...
#include "open_spiel/games/leduc_poker_10_card.h"
...
namespace leduc_poker_10_card {
namespace {

constexpr double kAnte = 1;

const GameType kGameType{/*short_name=*/"leduc_poker_10_card",
                         /*long_name=*/"Leduc Poker 10 Card",
...
std::shared_ptr<const Game> Factory(const GameParameters& params) {
  return std::shared_ptr<const Game>(new Leduc10CardGame(params));
}
...
class Leduc10CardObserver : public Observer {
 public:
  Leduc10CardObserver(IIGObservationType iig_obs_type)
  ...
  static void WriteObservingPlayer(const Leduc10CardState& state, int player,
                                   Allocator* allocator) {
  ...
  static void WriteSinglePlayerCard(const Leduc10CardState& state, int player,
                                    Allocator* allocator) {
  ...
  static void WriteAllPlayerCards(const Leduc10CardState& state,
                                  Allocator* allocator) {
  ...
  static void WriteCommunityCard(const Leduc10CardState& state,
                                 Allocator* allocator) {
  ...
  static void WriteBettingSequence(const Leduc10CardState& state,
                                   Allocator* allocator) {
  ...
  static void WritePotContribution(const Leduc10CardState& state,
                                   Allocator* allocator) {
  ...
  void WriteTensor(const State& observed_state, int player,
    ...
    auto& state = open_spiel::down_cast<const Leduc10CardState&>(observed_state);
...
Leduc10CardState::Leduc10CardState(std::shared_ptr<const Game> game, bool action_mapping,
    ...
    : State(game),
      ...
      deck_size_((game->NumPlayers() + 3) * kNumSuits),
...
int Leduc10CardState::CurrentPlayer() const {
  if (IsTerminal()) {
...
void Leduc10CardState::DoApplyAction(Action move) {
  if (IsChanceNode()) {
...
std::vector<Action> Leduc10CardState::LegalActions() const {
...
std::string Leduc10CardState::ActionToString(Player player, Action move) const {
  return GetGame()->ActionToString(player, move);
}

std::string Leduc10CardState::ToString() const {
  std::string result;
...
bool Leduc10CardState::IsTerminal() const {
  return remaining_players_ == 1 || (round_ == 2 && ReadyForNextRound());
}

std::vector<double> Leduc10CardState::Returns() const {
  if (!IsTerminal()) {
...
std::string Leduc10CardState::InformationStateString(Player player) const {
  const Leduc10CardGame& game = open_spiel::down_cast<const Leduc10CardGame&>(*game_);
  return game.info_state_observer_->StringFrom(*this, player);
}

// Observation is card then contribution of each players to the pot.
std::string Leduc10CardState::ObservationString(Player player) const {
  const Leduc10CardGame& game = open_spiel::down_cast<const Leduc10CardGame&>(*game_);
  return game.default_observer_->StringFrom(*this, player);
}

void Leduc10CardState::InformationStateTensor(Player player,
...
void Leduc10CardState::ObservationTensor(Player player,
                                   absl::Span<float> values) const {
  ContiguousAllocator allocator(values);
  const Leduc10CardGame& game = open_spiel::down_cast<const Leduc10CardGame&>(*game_);
...
std::unique_ptr<State> Leduc10CardState::Clone() const {
  return std::unique_ptr<State>(new Leduc10CardState(*this));
}

std::vector<std::pair<Action, double>> Leduc10CardState::ChanceOutcomes() const {
...
int Leduc10CardState::NextPlayer() const {
  ...
  SpielFatalError("Error in Leduc10CardState::NextPlayer(), should not get here.");
}

int Leduc10CardState::RankHand(Player player) const {
...
void Leduc10CardState::ResolveWinner() {
...
void Leduc10CardState::SequenceAppendMove(int move) {
  if (round_ == 1) {
...
void Leduc10CardState::Ante(Player player, int amount) {
  pot_ += amount;
  ante_[player] += amount;
  money_[player] -= amount;
}

std::vector<int> Leduc10CardState::padded_betting_sequence() const {
...
void Leduc10CardState::SetPrivate(Player player, Action move) {
...
std::unique_ptr<State> Leduc10CardState::ResampleFromInfostate(
...
int Leduc10CardState::NumObservableCards() const {
  return suit_isomorphism_ ? deck_.size() / 2 : deck_.size();
}

int Leduc10CardState::MaxBetsPerRound() const { return 3 * num_players_ - 2; }

Leduc10CardGame::Leduc10CardGame(const GameParameters& params)
    : Game(kGameType, params),
      num_players_(ParameterValue<int>("players")),
      total_cards_((num_players_ + 3) * kNumSuits),
  ...
  default_observer_ = std::make_shared<Leduc10CardObserver>(kDefaultObsType);
  info_state_observer_ = std::make_shared<Leduc10CardObserver>(kInfoStateObsType);
}

std::unique_ptr<State> Leduc10CardGame::NewInitialState() const {
  return absl::make_unique<Leduc10CardState>(shared_from_this(),
                                       /*action_mapping=*/action_mapping_,
                                       /*suit_isomorphism=*/suit_isomorphism_);
}

int Leduc10CardGame::MaxChanceOutcomes() const {
  if (suit_isomorphism_) {
...
std::vector<int> Leduc10CardGame::InformationStateTensorShape() const {
...
std::vector<int> Leduc10CardGame::ObservationTensorShape() const {
  // One-hot encoding for player number (who is to play).
  // 2 slots of cards (total_cards_ bits each): private card, public card
...
double Leduc10CardGame::MaxUtility() const {
...
double Leduc10CardGame::MinUtility() const {
  // In poker, the utility is defined as the money a player has at the end of
  // the game minus then money the player had before starting the game.
...
std::shared_ptr<Observer> Leduc10CardGame::MakeObserver(
    absl::optional<IIGObservationType> iig_obs_type,
    const GameParameters& params) const {
  if (params.empty()) {
    return std::make_shared<Leduc10CardObserver>(
...
std::string Leduc10CardGame::ActionToString(Player player, Action action) const {
...
TabularPolicy GetAlwaysFoldPolicy(const Game& game) {
  SPIEL_CHECK_TRUE(
      dynamic_cast<Leduc10CardGame*>(const_cast<Game*>(&game)) != nullptr);
