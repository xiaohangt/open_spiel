...
const GameType kGameType{/*short_name=*/"leduc_poker_dummy",
                         /*long_name=*/"Leduc Poker Dummy",
...
std::shared_ptr<const Game> Factory(const GameParameters& params) {
  return std::shared_ptr<const Game>(new LeducDummyGame(params));
}

REGISTER_SPIEL_GAME(kGameType, Factory);

std::string StatelessActionToString(Action action) {
  if (action % 3 == ActionType::kFold) {
    return "Fold";
  } else if (action % 3 == ActionType::kCall) {
    return "Call";
  } else if (action % 3 == ActionType::kRaise) {
    return "Raise";
  } else if (action >= 3) {
    return "Fold";
  } else {
...
class LeducObserver : public Observer {
 public:
  ...
  static void WriteObservingPlayer(const LeducDummyState& state, int player,
                                   Allocator* allocator) {
  ...
  static void WriteSinglePlayerCard(const LeducDummyState& state, int player,
                                    Allocator* allocator) {
  ...
  static void WriteAllPlayerCards(const LeducDummyState& state,
                                  Allocator* allocator) {
  ...
  static void WriteCommunityCard(const LeducDummyState& state,
                                 Allocator* allocator) {
  ...
  static void WriteBettingSequence(const LeducDummyState& state,
                                   Allocator* allocator) {
  ...
  static void WritePotContribution(const LeducDummyState& state,
                                   Allocator* allocator) {
  ...
  void WriteTensor(const State& observed_state, int player,
    ...
    auto& state = open_spiel::down_cast<const LeducDummyState&>(observed_state);
...
LeducDummyState::LeducDummyState(std::shared_ptr<const Game> game, bool action_mapping,
...
int LeducDummyState::CurrentPlayer() const {
  if (IsTerminal()) {
...
// underlying player.
// On a player node, it should be ActionType::{kFold, kCall, kRaise, kLose}
void LeducDummyState::DoApplyAction(Action move) {
  if (IsChanceNode()) {
    ...
    } else if (move % 3 == ActionType::kCall) {
      SPIEL_CHECK_NE(cur_player_, kChancePlayerId);
    ...
    } else if (move % 3 == ActionType::kRaise) {
      SPIEL_CHECK_NE(cur_player_, kChancePlayerId);
...
std::vector<Action> LeducDummyState::LegalActions() const {
  ...
  if (action_mapping_) {
    ...
    for (int i = 0; i<num_actions; i=i+1){
...
//    movelist.push_back(ActionType::kRaise);
  ...
  if (stakes_ > ante_[cur_player_]) {
    for (int i = 0; i<num_actions; i=i+3){
      movelist.push_back(i);
    }
//    movelist.push_back(ActionType::kFold);
...
//  movelist.push_back(ActionType::kCall);
  for (int i = 1; i<num_actions; i=i+3){
    movelist.push_back(i);
  }
...
//  movelist.push_back(ActionType::kCall);
  ...
  if (num_raises_ < 2) {
    for (int i = 2; i<num_actions; i=i+3){
      movelist.push_back(i);
    }
//    movelist.push_back(ActionType::kRaise);
  }
...
std::string LeducDummyState::ActionToString(Player player, Action move) const {
  return GetGame()->ActionToString(player, move);
}

std::string LeducDummyState::ToString() const {
  std::string result;
...
bool LeducDummyState::IsTerminal() const {
  return remaining_players_ == 1 || (round_ == 2 && ReadyForNextRound());
}

std::vector<double> LeducDummyState::Returns() const {
  if (!IsTerminal()) {
...
std::string LeducDummyState::InformationStateString(Player player) const {
  const LeducDummyGame& game = open_spiel::down_cast<const LeducDummyGame&>(*game_);
  return game.info_state_observer_->StringFrom(*this, player);
}

// Observation is card then contribution of each players to the pot.
std::string LeducDummyState::ObservationString(Player player) const {
  const LeducDummyGame& game = open_spiel::down_cast<const LeducDummyGame&>(*game_);
  return game.default_observer_->StringFrom(*this, player);
}

void LeducDummyState::InformationStateTensor(Player player,
...
void LeducDummyState::ObservationTensor(Player player,
                                   absl::Span<float> values) const {
  ContiguousAllocator allocator(values);
  const LeducDummyGame& game = open_spiel::down_cast<const LeducDummyGame&>(*game_);
...
std::unique_ptr<State> LeducDummyState::Clone() const {
  return std::unique_ptr<State>(new LeducDummyState(*this));
}

std::vector<std::pair<Action, double>> LeducDummyState::ChanceOutcomes() const {
...
int LeducDummyState::NextPlayer() const {
  ...
  SpielFatalError("Error in LeducDummyState::NextPlayer(), should not get here.");
}

int LeducDummyState::RankHand(Player player) const {
...
void LeducDummyState::ResolveWinner() {
...
void LeducDummyState::SequenceAppendMove(int move) {
  if (round_ == 1) {
...
void LeducDummyState::Ante(Player player, int amount) {
  pot_ += amount;
  ante_[player] += amount;
  money_[player] -= amount;
}

std::vector<int> LeducDummyState::padded_betting_sequence() const {
...
void LeducDummyState::SetPrivate(Player player, Action move) {
...
std::unique_ptr<State> LeducDummyState::ResampleFromInfostate(
...
int LeducDummyState::NumObservableCards() const {
  return suit_isomorphism_ ? deck_.size() / 2 : deck_.size();
}

int LeducDummyState::MaxBetsPerRound() const { return 3 * num_players_ - 2; }

LeducDummyGame::LeducDummyGame(const GameParameters& params)
...
std::unique_ptr<State> LeducDummyGame::NewInitialState() const {
  return absl::make_unique<LeducDummyState>(shared_from_this(),
                                       /*action_mapping=*/action_mapping_,
                                       /*suit_isomorphism=*/suit_isomorphism_);
}

int LeducDummyGame::MaxChanceOutcomes() const {
  if (suit_isomorphism_) {
...
std::vector<int> LeducDummyGame::InformationStateTensorShape() const {
...
std::vector<int> LeducDummyGame::ObservationTensorShape() const {
  // One-hot encoding for player number (who is to play).
  // 2 slots of cards (total_cards_ bits each): private card, public card
...
double LeducDummyGame::MaxUtility() const {
...
double LeducDummyGame::MinUtility() const {
  // In poker, the utility is defined as the money a player has at the end of
  // the game minus then money the player had before starting the game.
...
std::shared_ptr<Observer> LeducDummyGame::MakeObserver(
...
std::string LeducDummyGame::ActionToString(Player player, Action action) const {
...
TabularPolicy GetAlwaysFoldPolicy(const Game& game) {
  SPIEL_CHECK_TRUE(
      dynamic_cast<LeducDummyGame*>(const_cast<Game*>(&game)) != nullptr);
