...
from open_spiel.python.algorithms.cfr import _apply_regret_matching_plus_reset, _update_current_policy
import pyspiel
# Start
...
class _CFRSolverBase(object):
    ...
    def __init__(self, game, alternating_updates, linear_averaging,
        ...
        def info_state_nodes(self):
            return self._info_state_nodes

        def current_policy(self):
            return self._current_policy

        def initialize_info_state_nodes(self, state):
            # print("start init because of new br")
            self._initialize_info_state_nodes(state)
        ...
        def _initialize_info_state_nodes(self, state):
            ...
            # print(info_state, state.legal_actions(current_player))
        ...
        def get_infostate_policy(self, info_state_str):
            return self._get_infostate_policy(info_state_str)
...
class OnlineTraining:
    def __init__(self, game, solvers, linear_average_plus, xoso_id):
        self._game = game
        self.solvers = solvers
        self._linear_average_plus = linear_average_plus
        self._root_node = self._game.new_initial_state()
        self._iteration = 0
        self.change = 0
        self.xoso_id = xoso_id
        self._num_players = game.num_players()
    ...
    def evaluate_and_update_policy(self):
        ...
        for change in range(1):
            self.change = change
            self._root_node = self._game.new_initial_state()
            root_values.append(self.compute_counterfactual_regret_for_player(
                state=self._root_node,
                policies=None,
                reach_probabilities=np.ones(self._num_players + 1),
                player=None))
        if self._linear_average_plus:
            for solver in self.solvers:
                _apply_regret_matching_plus_reset(solver.info_state_nodes())

        # for solver in self.solvers:
        solver = self.solvers[self.xoso_id]
        _update_current_policy(solver.current_policy(), solver.info_state_nodes())

        return root_values

    def compute_counterfactual_regret_for_player(self, state, policies,
                                                  reach_probabilities, player):
        """Increments the cumulative regrets and policy for `player`.
    ...
    """
        ...
        if state.is_chance_node():
            ...
            for action, action_prob in state.chance_outcomes():
                ...
                new_reach_probabilities = reach_probabilities.copy()
                new_reach_probabilities[-1] *= action_prob
                state_value += action_prob * self.compute_counterfactual_regret_for_player(
                    new_state, policies, new_reach_probabilities, player)
            return state_value

        current_player = state.current_player()
        if self.change:
            current_solver = self.solvers[(current_player + 1) % 2]
        else:
            current_solver = self.solvers[current_player]
        info_state = state.information_state_string(current_player)
        ...
        # state. Therefore, the utilities are cached.
        children_utilities = {}
        info_state_node = current_solver.info_state_nodes()[info_state]
        # opponent
        if current_player != self.xoso_id:
            action = max(info_state_node.legal_actions)
            new_state = state.child(action)
            new_reach_probabilities = reach_probabilities.copy()
            child_utility = self.compute_counterfactual_regret_for_player(
                new_state,
                policies=policies,
                reach_probabilities=new_reach_probabilities,
                player=player)
        ...
        if policies is None:
            info_state_policy = current_solver.get_infostate_policy(info_state)
        else:
            info_state_policy = policies[current_player](info_state)
        ...
        for action, action_prob in info_state_policy.items():
            ...
            cfr_regret = counterfactual_reach_prob * (
                    children_utilities[action][current_player] - state_value_for_player)
            if self._linear_average_plus:
                info_state_node.cumulative_regret[action] += self._iteration * cfr_regret
        ...
        # if current_player == 1:
        # print("regret: ", info_state, info_state_node.cumulative_regret)
        # print("policy: ", info_state, info_state_node.cumulative_policy)
